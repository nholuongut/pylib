...
from nholuongut.utils import ERRORS, qquit, CodingError, log, isStr, log_option, support_msg
from nholuongut.utils import WarningError, CriticalError, UnknownError
...
class NagiosPlugin(CLI):
    """
    nholuongut.NagiosPlugin class for rapid development of Nagios Plugins in Python
    """
    ...
    def status(self, status):
        if status not in ERRORS:
            raise CodingError("invalid status '{0}' passed to nholuongut.NagiosPlugin.status()".format(status))
