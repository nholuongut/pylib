...
# Alpine:
#
#   apk add --no-cache git make && git clone https://github.com/nholuongut/pylib && cd pylib && make
#
# Debian / Ubuntu:
#
#   apt-get update && apt-get install -y make git && git clone https://github.com/nholuongut/pylib && cd pylib && make
#
# RHEL / CentOS:
#
#   yum install -y make git && git clone https://github.com/nholuongut/pylib && cd pylib && make
...
build: init
	...
	#if [ "$$(python -c 'import sys; sys.path.append("pylib"); import nholuongut; print(nholuongut.utils.getPythonVersion())')" = "2.6" ]; then $(SUDO_PIP) pip uninstall -y pylint; fi
...
test-common:
	...
	#   /home/travis/build/nholuongut/pylib/bash-tools/pytools_checks/pylib/test/test_utils.py
	# which is not the same as the test file we want to collect:
	#   /home/travis/build/nholuongut/pylib/test/test_utils.py
...
coverage:
	...
	@#coverage run --source=nholuongut -m test
	coverage run --source=nholuongut -m nose
	@#coverage run --source=nholuongut -m pytest
	coverage report
